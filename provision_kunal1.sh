...
download_from_s3() {
  ...
  if [[ -n "${AWS_ACCESS_KEY:-}" && -n "${AWS_SECRET_KEY:-}" ]]; then
    ...
    AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY}" AWS_SECRET_ACCESS_KEY="${AWS_SECRET_KEY}" aws --region "${AWS_REGION}" s3 cp "${s3path}" "${dest}" --no-progress || true
  else
...
# ---------- Permission & Dependency Fixes ----------
log "Ensuring core scripts are executable and dependencies installed..."

# Fix execute permissions
chmod +x "${GAME_LAUNCHER}" 2>/dev/null || log "⚠️  Missing: ${GAME_LAUNCHER}"
chmod +x "${SIGNALLER_DIR}/platform_scripts/bash/start.sh" 2>/dev/null || log "⚠️  Missing: start.sh"
chmod +x "${SIGNALLER_DIR}/platform_scripts/bash/start_with_turn.sh" 2>/dev/null || log "⚠️  Missing: start_with_turn.sh"
chmod +x "${SIGNALLER_REG_SCRIPT}" 2>/dev/null || true

# Run npm install for SignallingWebServer
if [[ -f "${SIGNALLER_DIR}/package.json" ]]; then
  log "Installing npm dependencies for SignallingWebServer..."
  pushd "${SIGNALLER_DIR}" >/dev/null
  npm ci || npm install || log "⚠️  npm install failed!"
  popd >/dev/null
else
  log "⚠️  No package.json found in ${SIGNALLER_DIR}"
fi
...
tmux send-keys -t "${SESSION_NAME}:turn" "echo 'Starting coturn...'; nohup ${TURN_CMD} > ${LOG_DIR}/turnserver.out 2>&1 & tail -f ${LOG_DIR}/turnserver.out" C-m
...
if [[ -x "${SIGNALLER_START_SCRIPT}" ]]; then
  ...
  tmux send-keys -t "${SESSION_NAME}:signaller" "cd ${SIGNALLER_DIR}; echo 'Starting signaller...'; nohup bash -lc '${SIGN_CMD}' > ${LOG_DIR}/signaller.log 2>&1 & tail -f ${LOG_DIR}/signaller.log" C-m
else
...
EOF
  ...
  cat > "${REG_SCRIPT}" <<EOF
#!/usr/bin/env bash
cd \$(dirname "${SIGNALLER_REG_SCRIPT}") || exit
