...
# awscli: prefer apt, fallback to pip
if [[ -n "${AWS_ACCESS_KEY:-}" && -n "${AWS_SECRET_KEY:-}" ]]; then
  if ! command -v aws >/dev/null 2>&1; then
    if apt-get install -y -qq awscli 2>/dev/null; then
      log "Installed awscli via apt."
    else
      log "Installing awscli via pip..."
      python3 -m pip install --upgrade --no-input awscli || log "pip install awscli failed (S3 downloads may fail)"
