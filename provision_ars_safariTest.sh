...
# Pixel flags (Safari requires H.264 BASELINE profile!)
...
# Wait for signaller to be ready before starting game (critical for Safari!)
echo "Waiting for signaller on port ${SPORT}..."
for attempt in \$(seq 1 60); do
  if ss -tln 2>/dev/null | grep -q ":${SPORT} " || netstat -tln 2>/dev/null | grep -q ":${SPORT} "; then
    echo "Signaller ready on port ${SPORT}, starting game..."
    sleep 3  # Extra buffer for Safari WebRTC negotiation
    break
  fi
  if [ "\$attempt" -eq 60 ]; then
    echo "WARNING: Signaller not detected on port ${SPORT}, starting anyway..."
  fi
  sleep 1
done
...
EOF
  ...
  # Wait a bit for game to start connecting before registering
  sleep 3
...
# Wait a bit to ensure game has connected to signaller
sleep 5
